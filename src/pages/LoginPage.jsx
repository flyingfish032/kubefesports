  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isSignUp) {
        ...
        const signupResp = await fetch("http://localhost:30083/back1/users/insert", {
          method: "POST",
        ...
        const loginResp = await fetch("http://localhost:30083/back1/users/signin", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            email: formData.email,
        ...
        if (code === "200" && token) {
          ...
          try {
            const usernameResp = await fetch("http://localhost:30083/back1/users/getusername", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
